...
namespace AdvectionDiffusion
{
   ...
   public: // Méthodes et opérateurs de la classe
      ...
      virtual double Neumann_value(const Eigen::VectorXd &X, const double t) const { return 0; };
      virtual double Dirichlet_value(const Eigen::VectorXd &X, const double t) const { return this->Exact_solution(X,t); };
   };

   class AdvectionHomogeneousNeumann: public Analytical {
   public:
      AdvectionHomogeneousNeumann(DataFile* data_file): Analytical(data_file) {}
      virtual double Exact_solution(const Eigen::VectorXd &X, const double t) const;
      virtual double Initial_condition(const Eigen::VectorXd &X) const;
      virtual Eigen::VectorXd Velocity(const Eigen::VectorXd &X, const double t) const;
   };

   class DiffusionHomogeneousNeumann: public Analytical {
   public:
      DiffusionHomogeneousNeumann(DataFile* data_file): Analytical(data_file) {}
      virtual double Exact_solution(const Eigen::VectorXd &X, const double t) const;
      virtual double Source_term(const Eigen::VectorXd &X, const double t) const;
   };

   class DiffusionAll: public Analytical {
   public:
      DiffusionAll(DataFile* data_file): Analytical(data_file) {}
      virtual double Exact_solution(const Eigen::VectorXd &X, const double t) const;
      virtual double Source_term(const Eigen::VectorXd &X, const double t) const;
      virtual double Neumann_value(const Eigen::VectorXd &X, const double t) const;
   };
